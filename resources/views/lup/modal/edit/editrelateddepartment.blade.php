...
<div class="modal fade" id="modaleditdepartment{{ $relateddepartment->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <!-- Modal Send Notif to Inisiator-->
<div class="modal fade" id="modalnotifdepartment{{ $relateddepartment->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md" role="utility">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Send Revise Notif to Inisiator</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="POST" name="upload-file" class="form-horizontal" enctype="multipart/form-data" id="upload-file" action="/lup/{{Crypt::encryptString($relateddepartment->id)}}/notifdepartment">
        @csrf           
        @method('put') 
      <div class="modal-body">  
        <div class="row mb-3">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Dismiss</button>
        <button type="submit" class="btn btn-primary" name="saveaction">Send</button>
      </div>
    </form>
    </div>
  </div>
